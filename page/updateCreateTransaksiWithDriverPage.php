...
<?php
    ...
    if(isset($_POST['create'])) {
        ...
        if($date1<$date3) {
            echo
            '<script>
            alert("Tanggal Waktu Mulai Sewa Tidak Boleh Lebih Kecil Dari Tanggal Waktu Sekarang!"); window.location = "./updateCreateTransaksiWithDriverPage.php?id='.$id.'"
            </script>';
        }else if($tanggal_waktu_sewa->days<1) {
            echo
            '<script>
            alert("Durasi Waktu Sewa Minimal Sehari!"); window.location = "./updateCreateTransaksiWithDriverPage.php?id='.$id.'"
            </script>';
        }else {
            $_SESSION['tanggal_waktu_mulai_sewa'] = $tanggal_waktu_mulai_sewa;
            $_SESSION['tanggal_waktu_selesai_sewa'] = $tanggal_waktu_selesai_sewa;

            echo '<script>window.location = "./updateListAsetMobilCustomerPage.php?stats=1&tanggal_waktu_sewa='.$tanggal_waktu_sewa->days.'&id='.$id.'"</script>';
        }
    ...
    <form action="./updateCreateTransaksiWithDriverPage.php?id='.$id.'" method="post">
        <div class="mb-3">
