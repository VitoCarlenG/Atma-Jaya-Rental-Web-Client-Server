    ...
    if(mysqli_num_rows($query) != 0) {
        ...
        if(isset($_POST['create'])) {
            ...
            if($date1<$date3) {
                echo
                '<script>
                alert("Tanggal Waktu Mulai Sewa Tidak Boleh Lebih Kecil Dari Tanggal Waktu Sekarang!"); window.location = "./createTransaksiWithDriverPage.php"
                </script>';
            }else if($tanggal_waktu_sewa->days<1) {
                echo
                '<script>
                alert("Durasi Waktu Sewa Minimal Sehari!"); window.location = "./createTransaksiWithDriverPage.php"
                </script>';
            }else {
                $_SESSION['tanggal_waktu_mulai_sewa'] = $tanggal_waktu_mulai_sewa;
                $_SESSION['tanggal_waktu_selesai_sewa'] = $tanggal_waktu_selesai_sewa;

                echo '<script>window.location = "./listAsetMobilCustomerPage.php?stats=1&tanggal_waktu_sewa='.$tanggal_waktu_sewa->days.'"</script>';
            }
        ...
        <form action="./createTransaksiWithDriverPage.php" method="post">
            <div class="mb-3">
